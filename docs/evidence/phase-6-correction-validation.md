...
The reproducible repo-source command for the stop case is `node scripts/run-pipeline.mjs --execute --input fixtures/po/FIX-02 --stop-before-publish`; recovery and no-op omit the stop flag. These commands require a fresh eligible session and do not authorize a fourth attempt today. **Documentation correction:** an earlier version incorrectly named `.lab/fixtures/FIX-02`, which is absent; exact original shell text was not retained, so these are reproduction instructions. Source hashes, attempts and cloud results remain recorded independently. At **02:47:12.547Z**, all five recorded first-attempt cloud jobs were inspected as DONE without errors before any resume. Serving inventory remained **10 snapshot rows / 10 event-evidence rows / one manifest**: only the prior FIX-01 batch was published. Its independent reconciliation still gave **48 remaining / $144**, while FIX-02's candidate had already passed. No publication job existed for the stopped attempt.
