...
Version: **v1.1 — accepted with conditions at Gate 4**. Prepared: **2026-09-20 America/Chicago**. Run: HDG-20260920-01. Decision owner: Human Manager. Basis: the approved OPP-02 [charter v1.1](../project-charter.md) and [Gate 3](../gates/gate-3-project-selection.md). The [Gate 4 decision](../gates/gate-4-architecture.md) records acceptance and the short-demonstration lifecycle. Only bounded project bootstrap has executed; no pipeline or report implementation is claimed. The [v1.0 submission](solution-architecture-v1.0-submitted.md) is preserved.

## Recommendation and management choices

Use an explicitly initiated **local Node.js batch runner**, **BigQuery** for the deployed analytical backend, and an **unpublished Power BI Desktop Import report**. Existing Node 24 is usable locally; SDK, authentication and report-host compatibility remain verification conditions. The manager reports that Power BI is installed, a new GCP project is needed, the desired region is “East US,” and none of the approximately $30/month allowance is used. Gate 4 accepts us-east4 and bounded setup. The dedicated project `astra-po-lab-20260921` now exists, but existing-account linkage failed on billing quota; [bootstrap evidence](../evidence/cloud-bootstrap.md) records the exact results. Deprecated `financial-analytics-demo` is excluded from all workload and quota contexts. Cloud Run, Cloud Scheduler, Cloud Storage, an always-on server and Power BI Service are outside the proposed baseline. The meaningful engineering work remains event reconciliation, repeatability, independent correctness, controlled publication and verified report behavior.

The manager accepted these three choices at [Gate 4](../gates/gate-4-architecture.md), subject to its conditions:
...
Gate 4 acceptance permits Phase 5 detailed design while billing/access/cost conditions remain blocking before dependent cloud work. Gate 5 must approve the analytics baseline before implementation. Bounded existing-billing setup is authorized; no new billing account may be created. Operate only long enough to gather approved pipeline/report/recovery evidence, preserve it and clean up; no 60-day operating period is intended.
