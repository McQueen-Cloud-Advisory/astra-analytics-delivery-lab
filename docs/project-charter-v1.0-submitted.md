...
Version: **v1.0 — proposed for Gate 3**. Prepared: **2026-09-20 America/Chicago**. Run: HDG-20260920-01. Status: **Draft; no project selected or charter approved**. Proposed initiative: **OPP-02**. Governing decision package: [Gate 3](gates/gate-3-project-selection.md).
...
Major dependencies are supported GCP access and billing, a viable Power BI host/connector/authentication route, the manager's availability, approved data contracts and a bounded incremental cost plan. Licensing, permissions and runtime installation are not inferred from the fictional scenario.
...
The B1 cloud component is provisionally **$2–12/month**, with a **$4–24 two-month development allowance**. Total incremental cash remains **Unknown** until licenses, agent/tool usage, actual services and access remediation are checked. These are estimates, not spending limits or authority. The fictional company benchmark of 10–18 engineering days/$8,000–14,400 labor and its modeled economics remain separate in [prioritization](portfolio/prioritization.md).
