...
Version: **v1.0**. Run: HDG-20260920-01. Prepared: **2026-09-20 America/Chicago**. Status: **Phase 3 analysis; Gate 3 selection pending**. Authority: [Gate 1 framework v1.0 / DEC-G1-001](../gates/gate-1-portfolio-review.md) and [Gate 2 package v1.0 / DEC-G2-001](../gates/gate-2-triage-review.md). Only approved survivors **OPP-01, OPP-02 and OPP-04** are scored.
...
Observed human effort, agent runtime and usage charges, installation/authentication work, Power BI verification, required licenses and deployment permissions remain **Unknown** unless separately evidenced. The [experiment delivery estimate](experiment-delivery-estimate.md) supplies separate, low-confidence one-person effort forecasts; they are not observations. No neutral score of 3 substitutes for unresolved actual feasibility or costs. They are separate conditions that can block or change selection regardless of the weighted comparison. Professional days multiplied by $100/hour are neither the user's cash expense nor an estimate of agent runtime; assumed owner-wait weeks below are not the user's schedule.
...
For [Gate 3](../gates/gate-3-project-selection.md), the decisive evidence is the **single person's ability to complete setup and verification**, the **actual incremental licenses/services needed**, and whether OPP-02's source contracts can stay as bounded as assumed. Synthetic data removes dependence on unavailable customer extracts while adding generator design and independent expected-result work. It does not remove platform feasibility or human verification requirements. If those conditions cannot be bounded, recommend **Additional Discovery** rather than a completed feasibility claim. The manager makes the selection and charter decision; architecture starts only after that approval.
